...
while(i < roomListXML.firstChild.childNodes.length)
{
   ...
   listGroup.scrollContent["item" + i].pro = roomListXML.firstChild.childNodes[i].attributes.pro;
   ...
   if(roomListXML.firstChild.childNodes[i].attributes.pro == "1")
   {
      listGroup.scrollContent["item" + i].bgColor._visible = true;
   }
   else
   {
   ...
   listGroup.scrollContent["item" + i].bg.onRelease = function()
   {
      ...
      if(this._parent.sm == 1)
      {
         trace(_global.newbieRoom);
         trace("setting newbie room to true!");
         _global.newbieRoom = true;
      }
      else
      {
         trace("setting newbie room to false!");
         trace(_global.newbieRoom);
         _global.newbieRoom = false;
      }
